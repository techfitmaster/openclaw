...
export function resolveSessionOptionGroups(
  ...
  // Always include the main session key for each configured agent so the
  // dropdown never loses agents after switching sessions.  A newly-created
  // agent may not have a session entry in the store yet, and the sessions
  // list is filtered by activeMinutes on initial load, so agent sessions
  // that have never been used (or haven't been active recently) would
  // otherwise disappear from the dropdown after a session switch.
  const agentMainKey = state.agentsList?.mainKey?.trim() || "main";
  for (const agent of state.agentsList?.agents ?? []) {
    if (agent.id) {
      addOption(`agent:${agent.id.trim().toLowerCase()}:${agentMainKey}`);
    }
  }
